...
export const db = {
  ...
  analytics: {
    ...
    // NEW: Get daily stage movements
    async getDailyStageMovements(startDate: Date, endDate: Date) {
      if (!isConnected) {
        ...
        const mockMovements = [
          {
            ...
            is_funnel_switch: false,
            total_movements: 15,
            movement_reasons: { "Progressed": 12, "Updated": 3 }
          },
          {
            movement_date: new Date().toISOString().split('T')[0],
            from_stage_name: "FU 3",
            to_stage_name: "BC 1",
            from_funnel: "follow_up",
            to_funnel: "broadcast",
            is_funnel_switch: true,
            total_movements: 8,
            movement_reasons: { "No Response": 8 }
          }
        ];
        return mockMovements;
      }
      ...
      try {
        ...
        console.error("Error in getDailyStageMovements:", error);
        return [];
      }
    },

    // NEW: Get lead journey analytics
    async getLeadJourneyAnalytics(leadId: string) {
      if (!isConnected) {
        // Mock data for offline mode
        return {
          lead_id: leadId,
      ...
      try {
        ...
        console.error("Error in getLeadJourneyAnalytics:", error);
        return null;
      }
    }
  },
