-- Fix: Cast UUID stage_id to TEXT
...
BEGIN
  ...
  WITH follow_up_stages AS (
    ...
    SELECT 
      s.id::TEXT AS id,  -- ✅ Cast UUID to TEXT
      s.stage_name::TEXT,
      s.stage_number,
      s.funnel_type::TEXT
    FROM stages s
  ...
  stage_entries AS (
    ...
    LEFT JOIN lead_stage_history h ON h.to_stage_id::TEXT = fs.id
  ...
  stage_progressions AS (
    ...
    FROM follow_up_stages fs
    LEFT JOIN lead_stage_history h ON h.from_stage_id::TEXT = fs.id
